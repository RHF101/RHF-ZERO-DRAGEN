...
RHF_MODE = (
    "Kamu adalah RHF Assistant. Jawab dengan santai, sopan, dan singkat. "
    "Jawab dengan cepat, tidak perlu menghasilkan kode tanpa diminta atau disuruh. "
...
def ask():
    ...
    with ThreadPoolExecutor(max_workers=max(1, len(pending_images) + len(pending_files) + 1)) as ex:
        ...
        history_future = ex.submit(fb_get_messages, session_id, mode, 10)
    ...
    payload = {
        ...
        # 1024 cukup buat mayoritas chat biasa dan bikin generation lebih
        # cepat kelar; kalau butuh jawaban panjang/kode, tetep bisa nembus
        # ini karena Mistral akan stop natural duluan pada respons pendek.
        "max_tokens": 1024,
    }
    if tools:
        payload["tools"] = tools
